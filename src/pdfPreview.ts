...
export class PdfPreview extends Disposable {
      ...
      webviewEditor.webview.onDidReceiveMessage(async (message) => {
        switch (message.command || message.type) {
            ...
            try {
              ...
              const errorMessage =
                error instanceof Error
                  ? error.message
                  : 'Unable to send PDF selection to Codex.';
          ...
          case 'translateSelection': {
            ...
            try {
              const translated = await translateAcademicSelection(message.text);
              await webviewEditor.webview.postMessage({
              ...
              await webviewEditor.webview.postMessage({
                ...
                error:
                  error instanceof Error
                    ? error.message
                    : 'Unable to translate PDF selection.',
              });
            }
            break;
          }
          case 'loadPdfAnnotations': {
            await this.sendPdfAnnotations();
            break;
          }
          case 'savePdfAnnotation': {
            try {
              ...
              vscode.window.showErrorMessage(
                error instanceof Error
                  ? error.message
                  : 'Unable to save PDF annotation.'
              );
            }
            break;
          }
          case 'createPdfAnnotationNote': {
            try {
              const store = await this.getNoteStore();
              ...
              vscode.window.showErrorMessage(
                error instanceof Error
                  ? error.message
                  : 'Unable to create PDF note.'
              );
            }
            break;
          }
          case 'openPdfAnnotationNote': {
            try {
              const store = await this.getNoteStore();
              ...
              vscode.window.showErrorMessage(
                error instanceof Error
                  ? error.message
                  : 'Unable to open PDF note.'
              );
            }
            break;
          }
          case 'deletePdfAnnotation': {
            try {
              const store = await this.getNoteStore();
              ...
              vscode.window.showErrorMessage(
                error instanceof Error
                  ? error.message
                  : 'Unable to delete PDF annotation.'
              );
            }
            break;
          }
  ...
  private getWebviewContents(): string {
    ...
    const resolveAsVersionedUri = (...p: string[]): string => {
      const uri = resolveAsUri(...p);
      return `${uri.toString()}?v=${Date.now()}`;
    };

    const config = vscode.workspace.getConfiguration();
    const getDefault = <T>(key: string): T | undefined =>
      (config.get(`paper-reader.${key}`) as T | undefined) ||
    ...
    const settings = {
      ...
      defaults: {
        ...
        scale: getDefault<string>('default.scale'),
        sidebar: getDefault<boolean>('default.sidebar'),
        scrollMode: getDefault<string>('default.scrollMode'),
        spreadMode: getDefault<string>('default.spreadMode'),
      },
    };
...
<script src="${resolveAsVersionedUri('lib', 'main.js')}"></script>
</head>`;
