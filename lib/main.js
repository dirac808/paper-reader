...
(function () {
  ...
  postMessage({ command: 'webviewProgress', stage: 'main-script-loaded' })
  ...
  window.addEventListener('load', async function () {
    postMessage({ command: 'webviewProgress', stage: 'window-loaded' })
    ...
    let awaitingFirstPageRender = false
    async function openPdf(stage) {
      awaitingFirstPageRender = true
      postMessage({ command: 'webviewProgress', stage })
      try {
        await PDFViewerApplication.open(config.path, loadOpts)
      } catch (error) {
        awaitingFirstPageRender = false
        showFatalError(
    ...
    PDFViewerApplication.initializedPromise.then(() => {
      postMessage({ command: 'webviewProgress', stage: 'pdfjs-initialized' })
  ...
  function showFatalError(message, source, line, column, error) {
    ...
    const location = source
      ? '\n' + source + ':' + (Number(line) || 0) + ':' + (Number(column) || 0)
      : ''
    const stack = error && error.stack ? '\n' + error.stack : ''
    msg.innerText = 'Paper Reader could not load this PDF: ' + detail + location + stack
    document.body = msg
  }

  window.onerror = showFatalError
